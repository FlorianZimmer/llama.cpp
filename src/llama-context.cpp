...
static bool mtp_backend_seed_debug_enabled();
...
llama_context::llama_context(
    ...
    if (native_mtp.desc.n_predict > 0) {
        LLAMA_LOG_INFO("%s: native_mtp_layers = %u\n", __func__, native_mtp.desc.n_predict);
        LLAMA_LOG_INFO("%s: native_mtp_dedicated_embeddings = %s\n", __func__, native_mtp.desc.dedicated_embeddings ? "true" : "false");
    }
...
void llama_context::synchronize() {
    ...
    native_mtp.clear_backend_capture_views();
...
const llama_mtp_desc & llama_context::mtp_desc() const {
    return native_mtp.desc;
}

bool llama_context::mtp_is_enabled() const {
    return native_mtp.enabled();
}

int32_t llama_context::mtp_draft_batch(
        ...
        const llama_pos    * pos,
               ...
               llama_token * draft,
               uint32_t      n_draft) {
    if (!native_mtp.enabled() || seq_ids == nullptr || tokens == nullptr || pos == nullptr || draft == nullptr || n_seq == 0 || n_draft == 0) {
        return 0;
    }

    // Finish the verifier decode and seed copy before reusing the scheduler for MTP.
    synchronize();
    ...
    if (native_mtp.seed_mode == LLAMA_MTP_SEED_MODE_NONE) {
        return 0;
    }
    ...
    switch (native_mtp.seed_mode) {
        ...
        case LLAMA_MTP_SEED_MODE_BACKEND:
            if (!native_mtp.seed_backend.ready()) {
                return 0;
            }

            if (mtp_backend_seed_debug_enabled()) {
                std::vector<float> debug_row(model.hparams.n_embd);
                for (uint32_t i = 0; i < n_seq; ++i) {
                    ggml_backend_tensor_get(native_mtp.seed_backend.seed_cache_rows[seq_ids[i]], debug_row.data(), 0, sizeof(float) * model.hparams.n_embd);
                    if (std::memcmp(debug_row.data(), native_mtp.seed_row(seq_ids[i]), sizeof(float) * model.hparams.n_embd) != 0) {
                        LLAMA_LOG_ERROR("%s: backend seed cache mismatch for seq_id=%d\n", __func__, seq_ids[i]);
                        return 0;
                    }
                }
            }
            ...
            for (uint32_t i = 0; i < n_seq; ++i) {
                ggml_backend_tensor_copy(
                        native_mtp.seed_backend.seed_cache_rows[seq_ids[i]],
                        native_mtp.seed_backend.seed_batch_rows[i]);
            }

            if (mtp_backend_seed_debug_enabled()) {
                std::vector<float> debug_row(model.hparams.n_embd);
                for (uint32_t i = 0; i < n_seq; ++i) {
                    ggml_backend_tensor_get(native_mtp.seed_backend.seed_batch_rows[i], debug_row.data(), 0, sizeof(float) * model.hparams.n_embd);
                    if (std::memcmp(debug_row.data(), native_mtp.seed_row(seq_ids[i]), sizeof(float) * model.hparams.n_embd) != 0) {
                        LLAMA_LOG_ERROR("%s: backend seed batch mismatch for seq_id=%d at batch row=%u\n", __func__, seq_ids[i], i);
                        return 0;
                    }
                }
            }

            mtp_seed_backend = native_mtp.seed_backend.seed_batch_dev;
            mtp_seed_generation = native_mtp.seed_backend.generation;
            break;
    }

    if (native_mtp.seed_mode == LLAMA_MTP_SEED_MODE_HOST && mtp_seed == nullptr) {
        return 0;
    }

    if (native_mtp.seed_mode == LLAMA_MTP_SEED_MODE_BACKEND && mtp_seed_backend == nullptr) {
        return 0;
    }
    ...
    for (uint32_t i = 0; i < n_seq; ++i) {
        ...
        for (uint32_t j = 0; j < ubatch.n_pos; ++j) {
            ubatch.pos[i * ubatch.n_pos + j] = pos[i];
        }
    ...
    const uint32_t n_outputs_save = n_outputs;
    n_outputs = n_seq;
    ...
    if (graph_reuse_disable || !res->can_reuse(gparams)) {
        ...
        if (gf == nullptr || !ggml_backend_sched_alloc_graph(sched.get(), gf)) {
    ...
    status = graph_compute(gf_res_prev->get_gf(), false);
    n_outputs = n_outputs_save;
    ...
    auto * t_mtp_tokens = gf_res_prev->get_mtp_tokens();
    if (t_mtp_tokens == nullptr) {
        return 0;
    }

    ggml_backend_t backend = ggml_backend_sched_get_tensor_backend(sched.get(), t_mtp_tokens);
    GGML_ASSERT(backend != nullptr);

    ggml_backend_tensor_get_async(backend, t_mtp_tokens, draft, 0, sizeof(*draft) * n_seq);
    synchronize();

    const uint32_t n_mtp = std::min<uint32_t>(n_draft, native_mtp.desc.n_draft);
...
int32_t llama_context::mtp_draft(llama_seq_id seq_id, llama_token token, llama_pos pos, llama_token * draft, uint32_t n_draft) {
    return mtp_draft_batch(&seq_id, &token, &pos, 1, draft, n_draft);
}
...
static ggml_context_ptr mtp_capture_ctx_init(size_t n_views) {
    ggml_init_params params = {
        /*.mem_size   =*/ std::max<size_t>(1, n_views) * ggml_tensor_overhead(),
        /*.mem_buffer =*/ nullptr,
        /*.no_alloc   =*/ true,
    };
...
static bool mtp_backend_seed_debug_enabled() {
    const char * env = getenv("LLAMA_MTP_BACKEND_SEED_DEBUG");
    return env && atoi(env) != 0;
}

static bool mtp_backend_seed_force_host() {
    const char * env = getenv("LLAMA_MTP_BACKEND_SEED_FORCE_HOST");
    return env && atoi(env) != 0;
}

static bool mtp_capture_seed_rows_host(
        ggml_tensor * tensor,
        const std::map<llama_seq_id, uint32_t> & seq_to_row,
        llama_mtp_state & dst,
        ggml_backend_sched_t sched,
    ...
    dst.set_seed_mode(LLAMA_MTP_SEED_MODE_HOST);
...
static bool mtp_capture_seed_rows_backend(
    ...
    if (mtp_backend_seed_force_host()) {
        return false;
    }
    ...
    if (ggml_backend_sched_get_n_copies(sched) != 1) {
        return false;
    }
    ...
    auto capture_ctx = mtp_capture_ctx_init(seq_to_row.size());
    if (!capture_ctx) {
        return false;
    }

    std::vector<std::pair<llama_seq_id, ggml_tensor *>> src_rows;
    src_rows.reserve(seq_to_row.size());

    for (const auto & [seq_id, row] : seq_to_row) {
        GGML_ASSERT(0 <= seq_id && seq_id < LLAMA_MAX_SEQ);

        auto * src_row = ggml_view_1d(capture_ctx.get(), tensor, dst.n_embd, row * tensor->nb[1]);
        if (src_row == nullptr || ggml_backend_view_init(src_row) != GGML_STATUS_SUCCESS) {
            return false;
        }

        src_rows.emplace_back(seq_id, src_row);
    }
    ...
    for (const auto & [seq_id, src_row] : src_rows) {
        ...
        if (mtp_backend_seed_debug_enabled()) {
            ggml_backend_tensor_get_async(backend, tensor, dst.seed_row(seq_id), seq_to_row.at(seq_id) * tensor->nb[1], ggml_nbytes(src_row));
        }
    }

    dst.set_seed_mode(LLAMA_MTP_SEED_MODE_BACKEND);
    dst.seed_backend.capture_ctxs.emplace_back(std::move(capture_ctx));

    return true;
}

static void capture_mtp_seed_rows(
        ggml_tensor * tensor,
        const std::map<llama_seq_id, uint32_t> & seq_to_row,
        llama_mtp_state & dst,
        ggml_backend_sched_t sched) {
    if (tensor == nullptr || !dst.enabled() || seq_to_row.empty()) {
        return;
    }
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        auto * t_logits    = res->get_logits();
        auto * t_embd_raw  = res->get_embd();
        auto * t_embd_user = cparams.embeddings ? t_embd_raw : nullptr;

        if (t_embd_user && res->get_embd_pooled()) {
            t_embd_user = res->get_embd_pooled();
        }
        ...
        // extract embeddings
        if (embd.data && t_embd_user && n_outputs > 0) {
            ggml_backend_t backend_embd = ggml_backend_sched_get_tensor_backend(sched.get(), t_embd_user);
            GGML_ASSERT(backend_embd != nullptr);
                ...
                case LLAMA_POOLING_TYPE_NONE:
                    {
                        ...
                        if (n_outputs) {
                            ...
                            ggml_backend_tensor_get_async(backend_embd, t_embd_user, embd_out, 0, n_outputs*n_embd_out*sizeof(float));
                        }
                    } break;
                case LLAMA_POOLING_TYPE_MEAN:
                case LLAMA_POOLING_TYPE_CLS:
                case LLAMA_POOLING_TYPE_LAST:
                    {
                        ...
                        for (uint32_t s = 0; s < ubatch.n_seqs_unq; ++s) {
                            ...
                            embd_seq_out[seq_id].resize(n_embd_out);
                            ggml_backend_tensor_get_async(backend_embd, t_embd_user, embd_seq_out[seq_id].data(), (n_embd_out*seq_idx)*sizeof(float), n_embd_out*sizeof(float));
                        }
                    } break;
                case LLAMA_POOLING_TYPE_RANK:
                    {
                        ...
                        for (uint32_t s = 0; s < ubatch.n_seqs_unq; ++s) {
                            ...
                            embd_seq_out[seq_id].resize(n_cls_out);
                            ggml_backend_tensor_get_async(backend_embd, t_embd_user, embd_seq_out[seq_id].data(), (n_cls_out*seq_idx)*sizeof(float), n_cls_out*sizeof(float));
                        }
                    } break;
                case LLAMA_POOLING_TYPE_UNSPECIFIED:
                    {
...
llm_graph_params llama_context::graph_params(
            ...
            const llama_memory_context_i * mctx,
                    ...
                    llama_mtp_seed_mode   mtp_seed_mode,
                        const float       * mtp_seed,
                        ggml_tensor       * mtp_seed_backend,
                          uint64_t          mtp_seed_generation,
                        const llama_token * mtp_tokens,
                          uint32_t          n_mtp) const {
    return {
        /*.arch        =*/ model.arch,
        ...
        /*.mctx        =*/ mctx,
        /*.cross       =*/ &cross,
        /*.mtp_seed_mode =*/ mtp_seed_mode,
        /*.mtp_seed    =*/ mtp_seed,
        /*.mtp_seed_backend =*/ mtp_seed_backend,
...
bool llama_memory_seq_rm_attn(
    ...
    if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
        return hybrid->get_mem_attn()->seq_rm(seq_id, p0, p1);
    }

    if (auto * hybrid_iswa = dynamic_cast<llama_memory_hybrid_iswa *>(mem)) {
        return hybrid_iswa->get_mem_attn()->seq_rm(seq_id, p0, p1);
    }
    ...
    if (auto * kv_iswa = dynamic_cast<llama_kv_cache_iswa *>(mem)) {
        return kv_iswa->seq_rm(seq_id, p0, p1);
    }

    return false;
}

bool llama_memory_seq_rm_recr(
        llama_memory_t mem,
          llama_seq_id seq_id,
             llama_pos p0,
             llama_pos p1) {
    ...
    if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
        return hybrid->get_mem_recr()->seq_rm(seq_id, p0, p1);
    }

    if (auto * hybrid_iswa = dynamic_cast<llama_memory_hybrid_iswa *>(mem)) {
        return hybrid_iswa->get_mem_recr()->seq_rm(seq_id, p0, p1);
    }

    if (auto * recr = dynamic_cast<llama_memory_recurrent *>(mem)) {
        return recr->seq_rm(seq_id, p0, p1);
    }

    return false;
}

bool llama_memory_seq_cp_recr(
        llama_memory_t mem,
          llama_seq_id seq_id_src,
          llama_seq_id seq_id_dst,
             llama_pos p0,
    ...
    if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
        hybrid->get_mem_recr()->seq_cp(seq_id_src, seq_id_dst, p0, p1);
        return true;
    }

    if (auto * hybrid_iswa = dynamic_cast<llama_memory_hybrid_iswa *>(mem)) {
        hybrid_iswa->get_mem_recr()->seq_cp(seq_id_src, seq_id_dst, p0, p1);
        return true;
    }
...
int32_t llama_native_mtp_draft_batch(
        llama_context * ctx,
  const llama_seq_id * seq_ids,
  const llama_token  * tokens,
  const llama_pos    * pos,
         ...
         llama_token * draft,
         uint32_t      n_draft) {
    return ctx->mtp_draft_batch(seq_ids, tokens, pos, n_seq, draft, n_draft);
}

int32_t llama_native_mtp_draft(
        llama_context * ctx,
                llama_seq_id seq_id,
                llama_token  token,
                llama_pos    pos,
                llama_token * draft,
                   uint32_t  n_draft) {
    return ctx->mtp_draft(seq_id, token, pos, draft, n_draft);
}
