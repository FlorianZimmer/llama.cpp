...
#  define LLAMA_LOG_INFO(...)  do { std::fprintf(stderr, __VA_ARGS__); } while (0)
#endif
#ifndef LLAMA_LOG_ERROR
#  define LLAMA_LOG_ERROR(...) do { std::fprintf(stderr, __VA_ARGS__); } while (0)
#endif
...
llama_xq_remat_result llama_xquant_remat_kv(
    ...
    // We want K,V as [T, d_out]. Depending on how Wk/Wv are stored in your GGUF,
    // dims can be [d_out, d_in] or [d_in, d_out]. Use runtime dims to choose.
    auto mul_to_Td = [&](ggml_tensor * W, ggml_tensor * XtT) -> ggml_tensor * {
        GGML_ASSERT(W && XtT);
        const int64_t w0 = W->ne[0], w1 = W->ne[1];      // ggml 2D: cols in [0], rows in [1]
        const int64_t x0 = XtT->ne[0], x1 = XtT->ne[1];  // XtT is [d_in, T]
        ggml_tensor * M = nullptr;                       // [d_out, T]
        if (w1 == x0) {
            // W:[d_out, d_in]  * Xt:[d_in, T] -> [d_out, T]
            M = ggml_mul_mat(ctx, W, XtT);
        } else if (w0 == x0) {
            // W^T:[d_out, d_in] * Xt:[d_in, T] -> [d_out, T]
            M = ggml_mul_mat(ctx, ggml_transpose(ctx, W), XtT);
        } else {
            LLAMA_LOG_ERROR("[xquant][dbg] remat: W dims incompatible with X: "
                            "W=[%lld,%lld] X=[%lld,%lld]\n",
                            (long long)w0, (long long)w1,
                            (long long)x0, (long long)x1);
            return nullptr;
        }
        // Return [T, d_out] for downstream
        return ggml_transpose(ctx, M);
    };

    ggml_tensor * Ktmp = mul_to_Td(Wk, Xt);
    ggml_tensor * Vtmp = mul_to_Td(Wv, Xt);
    if (!Ktmp || !Vtmp) {
        return R; // ok=false
    }
    ...
    LLAMA_LOG_INFO("[xquant][dbg] remat OK: K=[T=%lld,d=%lld] V=[T=%lld,d=%lld] "
                   "Wk=[%lld,%lld] Wv=[%lld,%lld]\n",
                   (long long)K->ne[0], (long long)K->ne[1],
                   (long long)V->ne[0], (long long)V->ne[1],
                   (long long)Wk->ne[0], (long long)Wk->ne[1],
                   (long long)Wv->ne[0], (long long)Wv->ne[1]);
