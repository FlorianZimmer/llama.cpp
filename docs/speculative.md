...
Some models expose a native multi-token prediction (MTP / NextN) head inside the same model.
In this mode, llama.cpp keeps a single verifier `llama_context`, samples the first next token from the normal verifier logits, and then uses the model's native MTP head to draft only the continuation token(s).
...
- the current runtime is intentionally single-step and drafts only one continuation token per verifier step.

Current limitation:

- native `mtp` can work functionally but still lose to greedy baseline if the verifier is already fast and the current one-token draft does not amortize enough work;
- hybrid/recurrent native-MTP paths can also diverge from greedy baseline under `-np > 1` on near-tie tokens because verifier numerics depend on batch shape.
